...
let lastCustomerKey='';const customerStatus=document.createElement('small');customerStatus.className='cep-status';customerStatus.setAttribute('aria-live','polite');form.elements.email.closest('label').append(customerStatus);
function applyCustomer(customer){if(!customer)return;form.elements.holder.value=customer.holder||form.elements.holder.value;form.elements.cpf.value=customer.cpf?customer.cpf.replace(/^(\d{3})(\d{3})(\d{3})(\d{2})$/,'$1.$2.$3-$4'):form.elements.cpf.value;form.elements.birth.value=customer.birth||form.elements.birth.value;form.elements.phone.value=customer.phone||form.elements.phone.value;if(customer.person==='pj'){form.elements.cnpj.value=customer.cnpj?customer.cnpj.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/,'$1.$2.$3/$4-$5'):form.elements.cnpj.value;form.elements.company.value=customer.company||form.elements.company.value;form.elements.tradeName.value=customer.tradeName||form.elements.tradeName.value}const a=customer.address||{};for(const name of ['zip','street','number','extra','district','city','state'])if(a[name])form.elements[name].value=a[name];customerStatus.textContent='Cadastro anterior encontrado. Confira os dados preenchidos automaticamente.'}
async function lookupKnownCustomer(){const cnpj=digits(form.elements.cnpj.value),cpf=digits(form.elements.cpf.value),document=person.value==='pj'&&cnpj.length===14?cnpj:cpf,email=form.elements.email.value.trim().toLowerCase();if(![11,14].includes(document.length)){customerStatus.textContent='';return}if(!/^\S+@\S+\.\S+$/.test(email)){customerStatus.textContent='Informe o e-mail já cadastrado para recuperar os dados com segurança.';return}const key=`${document}:${email}`;if(key===lastCustomerKey)return;lastCustomerKey=key;customerStatus.textContent='Procurando cadastro anterior…';try{const response=await fetch(`/api/customers/lookup?document=${encodeURIComponent(document)}&email=${encodeURIComponent(email)}`);const result=await response.json();if(!response.ok)throw new Error(result.message||'Não foi possível consultar.');if(result.customer)applyCustomer(result.customer);else customerStatus.textContent='Nenhum cadastro anterior localizado com este documento e e-mail.'}catch(error){lastCustomerKey='';customerStatus.textContent=error.message}}
form.elements.cpf.addEventListener('blur',lookupKnownCustomer);form.elements.cnpj.addEventListener('blur',lookupKnownCustomer);form.elements.email.addEventListener('blur',lookupKnownCustomer);

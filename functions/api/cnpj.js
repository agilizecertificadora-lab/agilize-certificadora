...
export async function onRequestGet({request}){
  ...
  try{
    ...
    if(response.ok){const company=await response.json();return Response.json({company:company.razao_social||'',tradeName:company.nome_fantasia||'',status:company.descricao_situacao_cadastral||''},{headers:{'Cache-Control':'public, max-age=3600'}})}
